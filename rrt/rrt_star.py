...
class RobotArm(NLinkArm):
    def get_points(self, joint_angle_list):
        ...
        return x_list, y_list, z_list # coordinates x,y,z
...
class RRTStar:
    ...
    def planning(self, animation=False, search_until_max_iter=False):
        ...
        for i in range(self.max_iter):
            ...
            rnd = self.get_random_node() # random node generate
            nearest_ind = self.get_nearest_node_index(self.node_list, rnd) # nearest node searching in node list
            new_node = self.steer(self.node_list[nearest_ind],
                                  rnd,
                                  self.expand_dis) # nearest node - rnd expand 

            if self.check_collision(new_node, self.robot, self.obstacle_list): # true = safe 
                near_inds = self.find_near_nodes(new_node) # find other possible nodes
