...
class NLinkArm:
    JOINT_LIMITS_RADIAN = [
        (math.radians(-166), math.radians(166)),  # Joint 1 limits in radians
        (math.radians(-101), math.radians(101)),  # Joint 2 limits in radians
        (math.radians(-166), math.radians(166)),  # Joint 3 limits in radians
        (math.radians(-176), math.radians(176)),  # Joint 4 limits in radians
        (math.radians(-166), math.radians(166)),  # Joint 5 limits in radians
        (math.radians(-1), math.radians(215)),    # Joint 6 limits in radians
        (math.radians(-166), math.radians(166))   # Joint 7 limits in radians
    ]
    ...
    def inverse_kinematics(self, ref_ee_pose, plot=False):
        for cnt in range(500):
            ee_pose = self.forward_kinematics() # [x,y,z,row,pitch,yaw]
    ...
    def euler_angle(self, target_H=None):
        trans = self.transformation_matrix()
    ...
    def set_joint_angles(self, joint_angle_list):
        for i in range(len(self.link_list)):
            min_limit,max_limit = self.JOINT_LIMITS_RADIAN[i]
            angle = max(min(joint_angle_list[i],max_limit),min_limit)
            self.link_list[i].dh_params_[0] = angle

    def update_joint_angles(self, diff_joint_angle_list):
        for i in range(len(self.link_list)):
            current_angle = self.link_list[i].dh_params_[0]
            new_angle=current_angle+diff_joint_angle_list[i]
            min_limit, max_limit = self.JOINT_LIMITS_RADIAN[i]
            new_angle = max(min(new_angle, max_limit), min_limit)
            self.link_list[i].dh_params_[0] = new_angle
